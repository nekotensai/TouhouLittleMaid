...
public class STTOpenAIClient implements STTClient {
    ...
    private final STTOpenAISite site;

    public STTOpenAIClient(HttpClient httpClient, STTOpenAISite site) {
    ...
    public void startRecord(STTConfig config, ResponseCallback<String> callback) {
        ...
        MicrophoneManager.startRecord(info.getName(), FORMAT, data -> {
            try {
                ...
                if (StringUtils.isNotBlank(this.site.getSecretKey())) {
                    builder.header(HttpHeaders.AUTHORIZATION, "Bearer " + this.site.getSecretKey());
                }
    ...
    private void handle(ResponseCallback<String> callback, HttpResponse<String> response, Throwable throwable, HttpRequest request) {
        this.<STTOpenAIMessage>handleResponse(callback, response, throwable, request,
                message -> callback.onSuccess(message.getText()),
                STTOpenAIMessage.class);
    }
